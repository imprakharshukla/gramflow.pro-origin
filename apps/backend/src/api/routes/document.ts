...
export default (app: Router) => {
  ...
  route.get("/label", async (req: Request, res: Response) => {
    const query = req.query;
    const order_ids = query.order_ids as string;
    if (!order_ids) {
    ...
    const labelFilePath = await documentServiceInstance.generatePDFFile(
      `http://localhost:3000/label?order_ids=${query.order_ids}`,
    );
    ...
    //   logger.info(
    //     `Generated label file: ${labelFilePath} at: ${labelFilePath}`,
    //   );
    res.download(labelFilePath);
  });
  route.get("/label", async (req: Request, res: Response) => {
    //   logger.info(
