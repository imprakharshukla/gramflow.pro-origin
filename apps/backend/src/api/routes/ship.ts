...
export default (server: ReturnType<typeof initServer>) => {
  ...
  return server.router(shipContract, {
    syncShipments: {
      middleware:
        [
          adminOnlyMiddleware<TsRestRequest<typeof shipContract.syncShipments>>
        ],
      handler: async ({ body }) => {
    ...
    createShipment: {
      middleware:
        [adminOnlyMiddleware<TsRestRequest<typeof shipContract.createShipment>>
        ],
      handler: async ({ body }) => {
        const orderServiceInstance = Container.get(OrderService);
    ...
    createPickup: {
      middleware:
        [
          adminOnlyMiddleware<TsRestRequest<typeof shipContract.createPickup>>
        ],
      handler: async ({ body }) => {
        const delhiveryServiceInstance = Container.get(DelhiveryService);
    ...
    calculateShippingCost: {
      middleware:
        [
          adminOnlyMiddleware<TsRestRequest<typeof shipContract.calculateShippingCost>>
        ],
      handler: async ({ query }) => {
        const delhiveryServiceInstance = Container.get(DelhiveryService);
        logger.debug(
    ...
    getPickup: {
      middleware:
        [
          adminOnlyMiddleware<TsRestRequest<typeof shipContract.getPickup>>
        ],
      handler: async ({ query }) => {
        const delhiveryServiceInstance = Container.get(DelhiveryService);
