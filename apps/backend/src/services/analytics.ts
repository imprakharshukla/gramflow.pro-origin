...
export default class AnalyticsService {
    ...
    public async getTopStatesAndCitiesAnalytics(): Promise<{
        ...
        const topStates = await db.$queryRaw<
            {
                state: string;
                id_count: number;
            }[]
        >`SELECT u.state, COUNT(u.id)::int AS id_count
  FROM "Users" u
  ...
  `
        ...
        const topCities = await db.$queryRaw<
            {
                city: string;
                id_count: number;
            }[]
        >`SELECT u.city, COUNT(u.id)::int AS id_count
    FROM "Users" u
    JOIN "Orders" o ON u.id = o.user_id
    GROUP BY u.city
    ORDER BY id_count DESC
