...
async function fetchMediaDataOfParent(
    ...
    for (const targetPermalink of permLinks) {
        let nextPage: string | null = `https://graph.instagram.com/v17.0/me/media?fields=id,media_url,permalink&access_token=${accessToken}`;
        while (nextPage) {
            const response = await fetch(nextPage);
            const data = await response.json();

            if (data.data) {
                const temp = data.data.find((item: MediaData) => item.permalink === targetPermalink);
                if (temp) {
                    const child = await fetchMediaDataWithPagination(temp, permaLinkWithIndex[k] ?? 1, accessToken);
...
async function fetchMediaDataWithPagination(
    ...
    let nextPage: string | null = `https://graph.instagram.com/v17.0/${mediaData.id}/children?fields=id,media_url,permalink&access_token=${accessToken}`;
    while (nextPage) {
        const response = await fetch(nextPage);
        const data = await response.json();
...
function extractIndexNumberFromUrl(url: string): number {
    const regex = /https:\/\/www\.instagram\.com\/p\/[a-zA-Z0-9_-]+\/\?img_index=([0-9]+)/;
    const match = url.match(regex);
    // @ts-ignore
...
export const fetchImageUrls = (urls: string[]): Promise<string[]> => {
    return new Promise((resolve, reject) => {
        ...
        const permalinks = urls.map(url => `https://www.instagram.com/p/${extractPostIdFromUrl(url)}/`);
        const permalinksWithIndex = urls.map(url => extractIndexNumberFromUrl(url));
